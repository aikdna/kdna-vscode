...
function copy(label) {
  const d = join(tmpdir(), `gc-${label}-${process.pid}-${Date.now()}`);
...
test('fails: metadata tar integrity mismatches', () => { const d = copy('w7'); const mp = join(d,'fixtures/runtime-candidates/kdna-core-0.21.0.tgz.info.json'); const m = JSON.parse(readFileSync(mp,'utf8')); m.tar_integrity = 'sha512-BAD'; writeFileSync(mp,JSON.stringify(m)); const r=run(d); rmSync(d,{recursive:true,force:true}); assert.ok(!r.pass); });
test('fails: metadata unpacked_sha256 tampered', () => { const d = copy('w8'); const mp = join(d,'fixtures/runtime-candidates/kdna-core-0.21.0.tgz.info.json'); const m = JSON.parse(readFileSync(mp,'utf8')); m.unpacked_sha256 = 'BADD'; writeFileSync(mp,JSON.stringify(m)); const r=run(d); rmSync(d,{recursive:true,force:true}); assert.ok(!r.pass); });
